...
export default function DaemonSets({ namespace }) {
  ...
  const fetchData = useCallback(async () => {
    if (!namespace) return;
    try {
      const res = await fetch(`/api/daemonsets/${namespace}`);
  ...
  const columns = [
    ...
    { header: 'Desired', accessor: 'desired' },
    { header: 'Current', accessor: 'current' },
    { header: 'Ready', accessor: 'ready' },
    { header: 'Available', accessor: 'available' },
    { header: 'Image', accessor: 'image', render: (row) => <span className="font-mono text-[10px] text-gray-400 truncate block max-w-[180px]">{row.image}</span> },
    { header: 'Age', accessor: 'age', render: (row) => <span className="text-gray-500 text-xs">{timeAgo(row.age)}</span> },
  ];

  return (
    <div>
      <h2 className="text-lg font-medium text-white mb-4">DaemonSets</h2>
      ...
      {yamlData && <YamlModal data={yamlData} title={`DaemonSet: ${yamlData.name}`} onClose={() => setYamlData(null)} />}
    </div>
  );
}
